 ...
 * with upper case modification on the first letter of each 
 * and other default names and rules for each special case.
 * 
 * @author Phuong
 * @version 1.0
 *
...
public class Name {
    ...
    /**
     * Private instance variable represents pseudo first name.
     */
    private String pseudoFirstName = "Alex";
    
    /**
     * Private instance variable represents pseudo middle name.
     */
    private String pseudoMidName = "Van";
    
    /**
     * Private instance variable represents pseudo last name.
     */
    private String pseudoLastName = "Smith";
    ...
    public Name(String first, String middle, String last) {
        if (!first.equals("") && !first.equals("\s")) {
            String modFirstname =  first.substring(0, 1).toUpperCase() 
                   + first.substring(1);
            first = modFirstname;
        } else {
            first = pseudoFirstName;
        }
        firstname = first;
        
        if (!middle.equals("") && !middle.equals("\s")) {
            String modMiddlename =  middle.substring(0, 1).toUpperCase() 
                    + middle.substring(1);
            middle = modMiddlename;
        } else {
            middle = pseudoMidName;
        }
        middlename = middle;
        
        if (!last.equals("") && !last.equals("\s")) {
            String modLastname =  last.substring(0, 1).toUpperCase() 
                    + last.substring(1);
            last = modLastname;
        } else {
            last = pseudoLastName;
     ...
     * in the full three part name. (nthChar means getCharacter)
